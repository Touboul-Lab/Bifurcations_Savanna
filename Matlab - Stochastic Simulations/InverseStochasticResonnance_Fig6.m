...
SIGMA_VECT=[0.001, 0.05, 0.01, 0.2]
...
for sigma=(SIGMA_VECT)
    ...
    for mmc=1:MC
        ...
        for k=2:Ntimes
            ...
            noiseG=sqrt(dt)*sigma*randn()*(mu*S(k-1) + nu*TT(k-1) + (f0+(f1-f0)/(1+exp(-(G(k-1)-t2)/s2)))*(1-(G(k-1)+S(k-1)+TT(k-1))) ...
                          +beta*G(k-1)*TT(k-1)+alpha*G(k-1)*(1-(G(k-1)+S(k-1)+TT(k-1))));
            noiseS=sqrt(dt)*sigma*randn()*(beta*G(k-1)*TT(k-1)-(w0 + (w1-w0)/(1+exp(-(G(k-1)-t1)/s1)))*S(k-1)-mu*S(k-1) ...
                                -alpha*S(k-1)*(1-(G(k-1)+S(k-1)+TT(k-1))));
            noiseT=sqrt(dt)*sigma*randn()*(beta*G(k-1)*TT(k-1)-(w0 + (w1-w0)/(1+exp(-(G(k-1)-t1)/s1)))*S(k-1)-mu*S(k-1) ...
                                -alpha*S(k-1)*(1-(G(k-1)+S(k-1)+TT(k-1))));
